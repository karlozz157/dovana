	...
	c = function () {
		...
		return r = {
			buttons: {
				...
				submit: '<button type="submit" class="alertify-button alertify-button-ok" id="alertify-ok"><span>{{ok}}</span></button>',
				ok: '<button class="alertify-button alertify-button-ok" id="alertify-ok"><span>CONTINUAR</span></button>',
				cancel: '<button class="alertify-button alertify-button-cancel" id="alertify-cancel"><span>CANCELAR</span></button>'
			},
			...
			build: function (a) {
				var b = "",
					...
					d = a.message,
					e = a.cssClass || "";
				switch (b += '<div class="alertify-dialog"><div><img src="'+base_url+'assets/site/images/logo_color.png" id="logo-alert"/></div><img src="'+base_url+'assets/site/images/dovana-letras-alert.png" id="dovana-letras-alert"/><span>[</span><p>El envío es por pedido. Si necesitas mandar a diferentes<br /> direcciones te pedimos hacer tus compras en pedidos separados.</p><span>]</span>', b += '<a id="alertify-resetFocusBack" class="alertify-resetFocus" href="#">Reset Focus</a>', "none" === q.buttonFocus && (b += '<a href="#" id="alertify-noneFocus" class="alertify-hidden"></a>'), "prompt" === c && (b += '<div id="alertify-form">'), b += '<article class="alertify-inner">', b += r.message.replace("{{message}}", d), "prompt" === c && (b += r.input), b += r.buttons.holder, b += "</article>", "prompt" === c && (b += "</div>"), b += '<a id="alertify-resetFocus" class="alertify-resetFocus" href="#">Reset Focus</a>', b += "</div>", c) {
				case "confirm":
